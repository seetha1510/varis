...
def main():
    ...
    f = open('UAT_7000-8000.csv', 'w', encoding='UTF8', newline='')
    ...
    limit = math.floor(multiprocessing.cpu_count() * 0.9)
    #print(limit)
    #print(seg_features)
    pool = multiprocessing.Pool()
    pool = multiprocessing.Pool( max(multiprocessing.cpu_count()//2, 1) )
    rows_to_write = pool.starmap(processAll, zip(range(7000,8000), repeat(skus), repeat(features), repeat(segments), repeat(seg_features), repeat(seg_ids), repeat(categories), repeat(categories_features_lookup), repeat(classes), repeat(classes_features_lookup), repeat(subclasses), repeat(subclasses_features_lookup), repeat(manufacturers), repeat(mfrPartNums)))
