...
def main():
    ...
    f = open('ClassificationAccuracy_2.csv', 'w', encoding='UTF8', newline='')
    ...
    # =============================================================================
    #Example of the function that gets called for each product
    #processAll(i, writer, skus, features, segments, seg_features, seg_ids, categories, categories_features_lookup, classes, classes_features_lookup, subclasses, subclasses_features_lookup, manufacturers, mfrPartNums)
    
    #This is the actual implimentation that executes processAll on all the items in parallel while preserving their order (which is why Pool is used)
    pstart_time = time.time()
    pool = multiprocessing.Pool()
    # zip(range(len(skus)) # Put this in as the first argument to run all items.
    rows_to_write = pool.starmap(processAll, zip(range(0,50), repeat(skus), repeat(features), repeat(segments), repeat(seg_features), repeat(seg_ids), repeat(categories), repeat(categories_features_lookup), repeat(classes), repeat(classes_features_lookup), repeat(subclasses), repeat(subclasses_features_lookup), repeat(manufacturers), repeat(mfrPartNums)))
    pool.close()
    pend_time = time.time()
